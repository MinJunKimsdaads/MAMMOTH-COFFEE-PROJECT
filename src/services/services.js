...
export const getDataFilteredSales = async (list) => {
    try{
       ...
       await Promise.all(
        list.map(async (i) => {
            const code = i.code;
            await fetch(`https://mammoth-coffee-project.vercel.app/api/sales?code=${code}`).then((result)=>{
                return result.json();
            }).then((data)=>{
...
export const get5DaysData = async (list) => {
    try{
       ...
       await Promise.all(
        list.map(async (i) => {
            const code = i.code;
            await fetch(`https://mammoth-coffee-project.vercel.app/api/indicator?code=${code}`).then((result)=>{
                return result.json();
            }).then((data)=>{
                const newData = {
                    ...i,
